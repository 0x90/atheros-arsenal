...
int draw_sample_ht20(Uint32 *pixels, struct scanresult *result, float startfreq, int highlight)
{
	...
	for (i = 0; i < SPECTRAL_HT20_NUM_BINS; i++) {
		...
		plot_datapoint(pixels, freq, startfreq, result->sample.ht20.noise,
			       ...
			       highlight);
		printf("pixels: %d, freq: %f, startfreq: %f, noise: %d, rssi: %d, data: %d, datasquaresum: %d, highlight: %d \n", 
				pixels, freq, startfreq, result->sample.ht20.noise, result->sample.ht20.rssi, data, datasquaresum, highlight);
